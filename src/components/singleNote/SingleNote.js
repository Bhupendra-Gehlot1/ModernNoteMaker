...
import { Check, Edit, Trash, Trash2 } from 'react-feather'
    ...
    <div className='px-2 col-lg-3 col-md-4 col-sm-6 h-100 mb-5'>
        <div className='card shadow px-2' style={{backgroundColor:`${item.background}`}}>
            ...
            <div className='content'>
                <textarea className='form-control text-center' disabled={!edit} value={content}  onChange={(e)=>setContent(e.target.value)} style={{backgroundColor: `${item.foreground}`}}>
                </textarea>

            </div>
            <section className='d-flex justify-content-between my-2'>
                {!edit ?
                <button className='btn nav-link px-3 py-1 btn-outline-warning btn-sm shadow' onClick={()=>setEdit(true)}><Edit/> Edit</button>
            :
            <button className='btn btn-outline-primary py-1 nav-link px-3 btn-sm shadow' onClick={handleEdit}><Check/></button>
            }

            <button className='btn btn-outline-danger py-1 nav-link px-3 btn-sm shadow' onClick={handleDelete}><Trash2/> Delete</button>
            </section>
        </div>
